...
function createHook(filename, script) {
    ...
    const name = path_1.default.basename(path_1.default.dirname(filename), '.d');
    // Check if hook exist
...
function getHooks(gitDir) {
    const gitHooksDir = path_1.default.join(gitDir, 'hooks_multi');
    return hookList.map((hookName) => path_1.default.join(gitHooksDir, `${hookName}.d/17_local_husky.sh`));
}
/**
